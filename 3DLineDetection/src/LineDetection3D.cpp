...
LineDetection3D::LineDetection3D(int thAngle,
				int orthoFactor,
				int radiusFactor)
{
	TH_ANGLE = thAngle;
	TH_ORTHO_FACTOR = orthoFactor;    // default was 1;
	TH_RADIUS_FACTOR = radiusFactor; // default was 50;
}
...
void LineDetection3D::pointCloudSegmentation( std::vector<std::vector<int> > &regions )
{
	...
	double thAngle = TH_ANGLE/180.0*CV_PI;
...
void LineDetection3D::regionGrow( double thAngle, std::vector<std::vector<int> > &regions )
{
	...
	for ( int i=0; i<idx; ++i )
	{
		...
		double thRadius2 = pow(TH_RADIUS_FACTOR*pcaInfos[idxStrater].scale, 2);
		...
		while( count < clusterTemp.size() )
		{
			...
			double thOrtho = TH_ORTHO_FACTOR * pcaInfos[idxSeed].scale;
